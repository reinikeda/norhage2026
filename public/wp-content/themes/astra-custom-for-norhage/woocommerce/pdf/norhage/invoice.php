...
<table class="head container">
	<tr>
		<td class="header">
			...
			<?php else : ?>
				<?php esc_html_e( 'Invoice', 'nh-theme' ); ?>
			<?php endif; ?>
		</td>
		<td class="shop-info">
		...
		<td class="order-data">
			<table>
				...
				<tr class="order-number">
					<th><?php esc_html_e( 'Order Number', 'nh-theme' ); ?></th>
					<td><?php $this->order_number(); ?></td>
				</tr>

				<tr class="order-date">
					<th><?php esc_html_e( 'Order Date', 'nh-theme' ); ?></th>
					<td><?php $this->order_date(); ?></td>
				</tr>

				<?php if ( $this->get_payment_method() ) : ?>
					<tr class="payment-method">
						<th><?php esc_html_e( 'Payment Method', 'nh-theme' ); ?></th>
...
<table class="order-details">
	<thead>
		<tr>
			<th class="product">
				<?php esc_html_e( 'Product', 'nh-theme' ); ?>
			</th>
			<th class="quantity">
				<?php esc_html_e( 'Quantity', 'nh-theme' ); ?>
			</th>
			<th class="price">
				<?php esc_html_e( 'Price', 'nh-theme' ); ?>
			</th>
		</tr>
	</thead>
	<tbody>
				...
				<td class="product">
					...
					<div class="item-meta">
						<?php if ( ! empty( $item['sku'] ) ) : ?>
							<p class="sku">
								<span class="label"><?php $this->sku_title(); ?></span>
				...
				<table class="totals">
					<tfoot>
						<?php foreach ( $this->get_woocommerce_totals() as $key => $total ) : ?>
							<?php
							// Default WooCommerce / plugin label.
							$label = $total['label'];

							// Override specific keys with our own translatable labels.
							switch ( $key ) {
								case 'cart_subtotal':
								case 'subtotal':
									$label = __( 'Subtotal', 'nh-theme' );
									break;

								case 'shipping':
									$label = __( 'Shipping', 'nh-theme' );
									break;

								case 'order_total':
								case 'total':
									$label = __( 'Total', 'nh-theme' );
									break;
							}
							?>
							<tr class="<?php echo esc_attr( $key ); ?>">
								<th class="description"><?php echo esc_html( $label ); ?></th>
								<td class="price">
									<span class="totals-price"><?php echo esc_html( $total['value'] ); ?></span>
								</td>
							</tr>
						<?php endforeach; ?>
					</tfoot>
