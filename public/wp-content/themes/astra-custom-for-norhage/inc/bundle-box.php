...
function nh_render_bundle_box() {
	...
	echo '<div class="nc-bundle-head"><h3 id="nc-bundle-title">' . esc_html__( 'Buy a complete set', 'nh-theme' ) . '</h3></div>';
	...
	foreach ( $rows as $r ) {
		...
		* ----------------------------------------- */
		...
		echo '      <span class="screen-reader-text">' . esc_html__( 'Minus Quantity', 'nh-theme' ) . '</span>';
		echo '      <a href="#" class="minus"' . ( $effective_max <= 0 ? ' aria-disabled="true"' : '' ) . '>-</a>';
		echo '      <label class="screen-reader-text" for="' . esc_attr( $input_id ) . '">' . esc_html__( 'Quantity', 'nh-theme' ) . '</label>';
		echo '      <input' . $qty_disabled . ' id="' . esc_attr( $input_id ) . '" type="number" inputmode="numeric" min="0"'
			 . $maxAttr
			 . ' step="1" value="0" name="quantity[' . esc_attr( $p->get_id() ) . ']"'
			 . ' class="input-text qty text"' . $dataMax . ' autocomplete="off">';
		echo '      <span class="screen-reader-text">' . esc_html__( 'Plus Quantity', 'nh-theme' ) . '</span>';
