...
add_action( 'woocommerce_after_shipping_rate', function( $rate, $index ) {
	...
	$hours    = esc_html__( 'Pickup hours: Mon–Fri 8:30–16:30', 'nh-theme' );
	$leadtime = esc_html__( 'You’ll receive an SMS/email.', 'nh-theme' );
	...
	echo '<div class="nh-pickup-details" role="region" aria-labelledby="' . esc_attr( $title_id ) . '">
			...
			<div class="nh-note">' . $hours . '</div>
			<div class="nh-note">' . $leadtime . '</div>
		  </div>';
...
add_action( 'woocommerce_after_shipping_rate', function ( WC_Shipping_Rate $rate, $index ) {
	...
	/* translators: 1: postcode, 2: country name */
	$line      = $postcode
		? sprintf( esc_html__( '%1$s, %2$s', 'nh-theme' ), $postcode, $country_n )
		: $country_n;
	...
	echo '<div class="nh-pickup-details"' . $style . ' role="region" aria-labelledby="' . esc_attr( $title_id ) . '">
			<strong id="' . esc_attr( $title_id ) . '">' . esc_html__( 'Shipping to', 'nh-theme' ) . '</strong>
...
function nh_show_total_cart_weight_row() {
	...
	<tr class="nh-cart-total-weight">
		<th><?php esc_html_e( 'Total weight', 'nh-theme' ); ?></th>
		<td data-title="<?php echo esc_attr__( 'Total weight', 'nh-theme' ); ?>">
