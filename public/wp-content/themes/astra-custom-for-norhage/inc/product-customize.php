...
function nh_cc_validate_custom_cut( $passed, $product_id, $qty = 0, $variation_id = 0 ) {
	...
	if ( $step > 0 ) {
		...
		if ( ( ( $w - $origin_w ) % $step ) !== 0 ) {
			wc_add_notice(
				sprintf( esc_html__( 'Width must align to %1$d mm steps starting at %2$d mm.', 'nh-theme' ), $step, $origin_w ),
				'error'
			);
			return false;
		}
		...
		if ( ( ( $l - $origin_l ) % $step ) !== 0 ) {
			wc_add_notice(
				sprintf( esc_html__( 'Length must align to %1$d mm steps starting at %2$d mm.', 'nh-theme' ), $step, $origin_l ),
				'error'
			);
			return false;
		}
