...
class NHGP_Overrides {
	...
	public static function apply( $rates, $package ) {
		...
		$tiers = array(
			...
			array( 'w' => (float) $heavy['t4_weight'], 'a' => (float) $heavy['t4_amount'], 'label' => 'L4' ),
			array( 'w' => (float) $heavy['t5_weight'], 'a' => (float) $heavy['t5_amount'], 'label' => 'L5' ),
		);

		$any_tier = false;
		foreach ( $tiers as $t ) {
			if ( $t['w'] > 0 ) { $any_tier = true; break; }
		}

		$chosen = null;

		// Heavy feature is ALWAYS on now – only check if there is at least one tier.
		if ( $any_tier ) {
			foreach ( $tiers as $t ) {
				if ( $total_weight >= $t['w'] && $t['w'] > 0 ) {
					$chosen = $t;
				}
			}
		}
		...
		foreach ( $rates as $key => $rate ) {
			...
			if ( method_exists( $rate, 'get_label' ) && method_exists( $rate, 'set_label' ) ) {
				$clean = preg_replace( '/\s*\(Heavy\s+L[1-5]\)\s*$/i', '', $rate->get_label() );
			...
			if ( $chosen ) {
				...
				if ( method_exists( $rate, 'get_label' ) && method_exists( $rate, 'set_label' ) ) {
					...
					$suffix = sprintf( __( 'Heavy %s', NHGP_TEXTDOMAIN ), $chosen['label'] );
					$rate->set_label( $rate->get_label() . ' (' . $suffix . ')' );
				}

				continue;
			}

			/* ------------ UNDER HEAVY LEVEL 1: CLASS DOMINANCE ------------ */
			...
			// Append class label (always, for visibility)
			if ( $best_tid && method_exists( $rate, 'get_label' ) && method_exists( $rate, 'set_label' ) ) {
