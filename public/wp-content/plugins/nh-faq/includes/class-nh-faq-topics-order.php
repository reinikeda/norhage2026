...
class NH_FAQ_Topics_Order {
    ...
    public function enqueue_admin_assets( $hook ) {
        ...
        if ( empty( $_GET['taxonomy'] ) || $_GET['taxonomy'] !== 'nh_faq_topic' ) {
            return;
        }
    ...
    public function ajax_save_order() {
        ...
        foreach ( $order as $term_id ) {
            $tid = (int) $term_id;
            if ( $tid > 0 ) {
                update_term_meta( $tid, self::META_KEY, $i++ );
            }
        }
    ...
    public function force_terms_order( $args, $taxonomies ) {
        if ( ! in_array( 'nh_faq_topic', (array) $taxonomies, true ) ) {
            return $args;
        }
        // Respect manual sorting in admin list table if "orderby" is set.
