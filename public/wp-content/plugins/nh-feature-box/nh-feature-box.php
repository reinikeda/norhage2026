 ...
 * Description: Global feature library (icon + label) and per-product picker (max 8). Renders right after short description on product pages.
 * Version:     1.0.1
...
class NH_Feature_Box {
    ...
    /** guard to avoid double render when we hook twice for safety */
    ...
    public function __construct() {
        ...
        /**
         * Frontend render:
         * - Primary: immediately after the short description (excerpt) at priority 21
         * - Fallback: before add-to-cart form in case a theme/plugin moves blocks around
         */
        add_action('woocommerce_single_product_summary', [$this, 'render_feature_box_after_excerpt'], 21);
        add_action('woocommerce_before_add_to_cart_form', [$this, 'render_feature_box_after_excerpt'], 1);
    ...
    public function add_feature_metabox() {
        add_meta_box('nhf_feature_meta', __('Feature Settings', 'nhf'), [$this, 'feature_metabox_html'], self::CPT, 'normal', 'high');
    }
    ...
    public function feature_metabox_html($post) {
        ...
        <div class="nhf-field" style="margin-top:12px;">
            <label><input type="checkbox" name="nhf_active" value="yes" <?php checked($active); ?>> <?php _e('Active (available in product picker)', 'nhf'); ?></label>
        </div>
        <p class="description"><?php _e('Tip: Keep labels short and clear. The feature title is used as the label on the product page.', 'nhf'); ?></p>
        <?php
    }
    ...
    public function add_product_metabox() {
        add_meta_box('nhf_product_features', __('Product Feature Box', 'nhf'), [$this, 'product_metabox_html'], 'product', 'side', 'default');
    }
    ...
    public function product_metabox_html($post) {
        ...
        <div class="nhf-product-box">
            <p><label><input type="checkbox" name="nhf_show_box" value="yes" <?php checked($show); ?>> <?php _e('Show Feature Box on this product', 'nhf'); ?></label></p>
            ...
            <div class="nhf-picker" <?php if (!$show) echo 'style="display:none"'; ?>>
                ...
                <p class="description"><?php _e('Checked items are active; drag to change order. Limit: 8.', 'nhf'); ?></p>
            </div>
        </div>
        <?php
    }
    ...
    private function get_product_features($product_id) {
        ...
        // Filter active and keep saved order
        $out = [];
        foreach ($ids as $fid) {
            $p = get_post($fid);
            if (!$p || $p->post_type !== self::CPT) continue;
    ...
    public function render_feature_box_after_excerpt() {
        if ($this->rendered || !is_product()) return;
        ...
        do_action('nh_after_feature_box', $product_id);
